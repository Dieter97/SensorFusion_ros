...
document node(the complete tree)
...
for
i = 0
:trackletsElement.getChildNodes.getLength-1 %
all children
of 'tracklets'
...
.getTagName, 'item_version') % meta-data
% fprintf( '%d\n',
str2double(element
...
tracklet.
h = str2double(element.getElementsByTagName('h').item(0).getTextContent);
tracklet.
w = str2double(element.getElementsByTagName('w').item(0).getTextContent);
tracklet.
l = str2double(element.getElementsByTagName('l').item(0).getTextContent);

tracklet.
first_frame =
...
str2double(element
.getElementsByTagName('first_frame').item(0).getTextContent);

poses = element.getElementsByTagName('poses').item(0);
tracklet.
poses = extractPoses(poses);

tracklet.
finished =
...
str2double(element
.getElementsByTagName('finished').item(0).getTextContent);
