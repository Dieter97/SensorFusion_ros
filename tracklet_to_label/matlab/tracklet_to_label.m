...
while
img_idx ~ = nimages
fileID = fopen(sprintf('/home/dieter/Documents/Kitti/benchmark/cpp/data/object/label_2/%06d.txt', img_idx), 'a');
fclose(fileID);
%
compute bounding
boxes for
...
in velodyne
coordinate system
corners_3D = R * [corners(it).x;
corners(it)
.
y;
corners(it)
.z];
corners_3D(1, :) = corners_3D(1, :) + t{
it
}(1, pose_idx);
corners_3D(2, :) = corners_3D(2, :) + t{
it
}(2, pose_idx);
corners_3D(3, :) = corners_3D(3, :) + t{
it
}(3, pose_idx);

corners_3D = (veloToCam { cam + 1 }
...
coordinates in
velodyneCS
        orientation_3D = R * [0.0, 0.7 * l;
0.0, 0.0; 0.0, 0.0];
orientation_3D(1, :) = orientation_3D(1, :) + t{
it
}(1, pose_idx);
orientation_3D(2, :) = orientation_3D(2, :) + t{
it
}(2, pose_idx);
orientation_3D(3, :) = orientation_3D(3, :) + t{
it
}(3, pose_idx);

orientation_3D = (veloToCam { cam + 1 }
...
%
drawBox3D(gh, occlusion{it}
(pose_idx), corners_2D, face_idx, orientation_2D)
...
to label
file
        fileID = fopen(sprintf('/home/dieter/Documents/Kitti/benchmark/cpp/data/object/label_2/%06d.txt', img_idx),
                       'a');
%
Object label
fprintf(fileID,
